...
class MailerProcessor{
	...
	public function processHtml($model,$str,$options = []){
        $mergeData = $this->mergeDataArray($model);
            ...
            */
            if (empty($options[$value])) {
	            if($this->inString($needle,$str)){
                    $str = str_replace($needle, ArrayHelper::getValue($mergeData,$value,''),$str);
                }
            }else{
            	if($this->inString($needle,$str)){
    ...
    protected function mergeDataArray($user){
        return [
            'FNAME'         => $user->profile->first_name,
            'LNAME'         => $user->profile->last_name,
            'FULLNAME'      => $user->fullName,
            'HOME_URL'      => Yii::$app->urlManager->createAbsoluteUrl(['/']),
